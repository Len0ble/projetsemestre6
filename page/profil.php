...
$user = $transaction->getuserById($_SESSION['User']['id']);
if(isset($_POST) && isset($_POST['click'])){
  $nom=$_POST['nom'];
  $prenom=$_POST['prenom'];
  $adresse=$_POST['adresse'];
  $tel=$_POST['tel'];
  $result = $transaction->updateprofil($_SESSION['User']['id'],$nom,$prenom,$adresse,$tel);
  ...
  <a class="navbar-brand" href="#"><img src="../assets/image/logoo.png"class="logo"></a>
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link" href="profil.php"><i class="bi bi-person-check-fill"></i>Profil</a>
        </li>
        <li class="nav-item">
...
<div class="container-fluid">
<form action="profil.php?idprofil=<?=$user[$_SESSION['User']['id']]?>" method="POST" class="row g-3 boutiquierform">
  <div class="col-md-6">
    <label for="Nom" class="form-label">Nom</label>
    <input name="nom" value="<?=$user['nom']?> "type="texte" class="form-control" id="Nom" required> 
  </div>
  <div class="col-md-6">
    <label for="Prenom" class="form-label">Prenom</label>
    <input name="prenom"value="<?=$user['prenom']?> "type="texte" class="form-control" id="Prenom" required>
  </div>
  <div class="col-6">
    <label for="inputAddress" class="form-label">Adresse</label>
    <input name="adresse" value="<?=$user['adresse']?> "type="text" class="form-control" id="inputAddress" placeholder="Dakar"required>
  </div>
  <div class="col-md-6">
    <label for="Téléphone" class="form-label">Téléphone</label>
    <input name="tel" value="<?=$user['tel']?>" type="number" class="form-control" id="Téléphone"required>
  </div>
  <div class="col-12">
    <button name="click" type="submit" class="btn btn-primary">Mis à jour</button>
  </div>
</form>
</div>
